...
#ifdef  CCS_DESIGN_FUNC_test_orig
extern void mc_testbench_capture_IN( ac_channel<pack<ac_int<8, true > > > &a_in,  ac_channel<pack<ac_int<8, true > > > &b_in, pack<bool > valid_in,  ac_channel<ac_int<20, true > > &result);
extern void mc_testbench_capture_OUT( ac_channel<pack<ac_int<8, true > > > &a_in,  ac_channel<pack<ac_int<8, true > > > &b_in, pack<bool > valid_in,  ac_channel<ac_int<20, true > > &result);
extern void mc_testbench_wait_for_idle_sync();

#define ccs_intercept_test_orig_5 \
  ccs_real_test_orig(ac_channel<pack<ac_int<8, true > > > &a_in,ac_channel<pack<ac_int<8, true > > > &b_in,pack<bool > valid_in,ac_channel<ac_int<20, true > > &result);\
  void test_orig(ac_channel<pack<ac_int<8, true > > > &a_in,ac_channel<pack<ac_int<8, true > > > &b_in,pack<bool > valid_in,ac_channel<ac_int<20, true > > &result)\
{\
    static bool ccs_intercept_flag = false;\
    if (!ccs_intercept_flag) {\
      std::cout << "SCVerify intercepting C++ function 'test_orig' for RTL block 'test_orig'" << std::endl;\
      ccs_intercept_flag=true;\
    }\
    mc_testbench_capture_IN(a_in,b_in,valid_in,result);\
    ccs_real_test_orig(a_in,b_in,valid_in,result);\
    mc_testbench_capture_OUT(a_in,b_in,valid_in,result);\
}\
  void ccs_real_test_orig
#else
#define ccs_intercept_test_orig_5 test_orig
#endif //CCS_DESIGN_FUNC_test_orig
