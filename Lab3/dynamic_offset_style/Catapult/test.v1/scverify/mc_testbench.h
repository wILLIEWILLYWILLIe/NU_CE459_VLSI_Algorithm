// ----------------------------------------------------------------------------
// SystemC Testbench Header
//
//    HLS version: 10.4a/835166 Production Release
//       HLS date: Thu Sep  5 21:35:46 PDT 2019
//  Flow Packages: HDL_Tcl 8.0a, SCVerify 10.4
//
//   Generated by: gel8580@finagle.wot.ece.northwestern.edu
// Generated date: Thu Nov 06 11:42:00 CST 2025
//
// ----------------------------------------------------------------------------
#ifdef CCS_SCVERIFY

// 
// -------------------------------------
// mc_testbench
// SCVerify mc_testbench SC_MODULE
// -------------------------------------
// 
#ifndef INCLUDED_MC_TESTBENCH_H
#define INCLUDED_MC_TESTBENCH_H


#ifndef SC_USE_STD_STRING
#define SC_USE_STD_STRING
#endif

#include <ac_int.h>
#include "ccs_testbench.h"
#include <systemc.h>
#include <tlm.h>
#include <mc_container_types.h>
#include <mc_typeconv.h>
#include <mc_transactors.h>
#include <mc_comparator.h>
#include <mc_end_of_testbench.h>
#include <vector>


class mc_testbench : public sc_module
{
public:
   // Interface Ports
   sc_in< bool > clk;
   sc_port< tlm::tlm_fifo_put_if< mgc_sysc_ver_array1D<int,40> > > ccs_din;
   sc_port< tlm::tlm_fifo_put_if< mc_wait_ctrl > > ccs_wait_ctrl_din;
   sc_port< tlm::tlm_fifo_put_if< ac_int<6, false > > > ccs_offset;
   sc_port< tlm::tlm_fifo_put_if< mc_wait_ctrl > > ccs_wait_ctrl_offset;
   sc_port< tlm::tlm_fifo_get_if< mgc_sysc_ver_array1D<int,40> > > ccs_dout;
   sc_port< tlm::tlm_fifo_put_if< mc_wait_ctrl > > ccs_wait_ctrl_dout;
   sc_in< bool > design_is_idle;
   sc_out< sc_logic > enable_stalls;
   sc_in< unsigned short > stall_coverage;
   
   // Data objects
   bool testbench_ended;
   int main_exit_code;
   bool atleast_one_active_input;
   sc_time last_event_time;
   sc_time last_event_time2;
   sc_signal< bool >                          cpp_testbench_active;
   sc_event testbench_end_event;
   sc_event reset_request_event;
   bool _checked_results;
   bool _failed;
   static mc_testbench* that;
   bool _capture_din;
   int din_capture_count;
   int din_iteration_count;
   bool _capture_offset;
   int offset_capture_count;
   int offset_iteration_count;
   tlm::tlm_fifo< mc_golden_info< mgc_sysc_ver_array1D<int,40>, int > > dout_golden;
   bool _capture_dout;
   int dout_capture_count;
   int dout_iteration_count;
   int wait_cnt;
   
   // Named Objects
   
   // Module instance pointers
   mc_comparator< mgc_sysc_ver_array1D<int,40> , int > *dout_comp;
   
   // Declare processes (SC_METHOD and SC_THREAD)
   void wait_for_end();
   void run();
   
   // Constructor
   SC_HAS_PROCESS(mc_testbench);
   mc_testbench(
      const sc_module_name& name
   )
      : clk("clk")
      , ccs_din("ccs_din")
      , ccs_wait_ctrl_din("ccs_wait_ctrl_din")
      , ccs_offset("ccs_offset")
      , ccs_wait_ctrl_offset("ccs_wait_ctrl_offset")
      , ccs_dout("ccs_dout")
      , ccs_wait_ctrl_dout("ccs_wait_ctrl_dout")
      , design_is_idle("design_is_idle")
      , enable_stalls("enable_stalls")
      , stall_coverage("stall_coverage")
      , cpp_testbench_active("cpp_testbench_active")
      , dout_golden("dout_golden",-1)
   {
      // Instantiate other modules
      dout_comp = new mc_comparator< mgc_sysc_ver_array1D<int,40> , int > (
         "dout_comp",
         "dout",
         0,
         0,
         1
      );
      dout_comp->data_in(ccs_dout);
      dout_comp->data_golden(dout_golden);
      
      
      // Register processes
      SC_METHOD(wait_for_end);
      sensitive << clk.pos() << testbench_end_event;
      SC_THREAD(run);
      // Other constructor statements
      set_stack_size(64000000);
      _checked_results = false;
      that = this;
      testbench_ended = false;
      main_exit_code = 0;
      atleast_one_active_input = true;
      _failed = false;
      _capture_din = true;
      _capture_offset = true;
      _capture_dout = true;
      wait_cnt = 0;
   }
   
   ~mc_testbench()
   {
      delete dout_comp;
      dout_comp = 0;
   }
   
   // C++ class functions
   public:
      static void wait_for_idle_sync() ;
   public:
      static void set_enable_stalls(bool flag) ;
   public:
      void reset_request() ;
   public:
      void capture_din( int din[40]) ;
   public:
      void capture_offset(ac_int<6, false > offset) ;
   public:
      void capture_dout( int dout[40]) ;
   protected:
      void wait_on_input_required() ;
   public:
      static void capture_IN( int din[40], ac_int<6, false > offset,  int dout[40]) ;
   public:
      static void capture_OUT( int din[40], ac_int<6, false > offset,  int dout[40]) ;
   public:
      static void exec_test( int din[40], ac_int<6, false > offset,  int dout[40]) ;
   protected:
      void start_of_simulation() ;
   protected:
      void end_of_simulation() ;
   public:
      void check_results() ;
   public:
      bool failed() ;
   public:
      void set_failed(bool fail) ;
};
#endif
#endif //CCS_SCVERIFY
