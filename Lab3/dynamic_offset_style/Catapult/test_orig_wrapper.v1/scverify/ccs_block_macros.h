...
#ifdef  CCS_DESIGN_FUNC_test_orig_wrapper
extern void mc_testbench_capture_IN( int din_in[40], ac_int<6, false > offset,  int dout[40]);
extern void mc_testbench_capture_OUT( int din_in[40], ac_int<6, false > offset,  int dout[40]);
extern void mc_testbench_wait_for_idle_sync();

#define ccs_intercept_test_orig_wrapper_23 \
  ccs_real_test_orig_wrapper(int din_in[40],ac_int<6, false > offset,int dout[40]);\
  void test_orig_wrapper(int din_in[40],ac_int<6, false > offset,int dout[40])\
{\
    static bool ccs_intercept_flag = false;\
    if (!ccs_intercept_flag) {\
      std::cout << "SCVerify intercepting C++ function 'test_orig_wrapper' for RTL block 'test_orig_wrapper'" << std::endl;\
      ccs_intercept_flag=true;\
    }\
    mc_testbench_capture_IN(din_in,offset,dout);\
    ccs_real_test_orig_wrapper(din_in,offset,dout);\
    mc_testbench_capture_OUT(din_in,offset,dout);\
}\
  void ccs_real_test_orig_wrapper
#else
#define ccs_intercept_test_orig_wrapper_23 test_orig_wrapper
#endif //CCS_DESIGN_FUNC_test_orig_wrapper
...
// test_orig 3 INLINE
#define ccs_intercept_test_orig_3 test_orig
