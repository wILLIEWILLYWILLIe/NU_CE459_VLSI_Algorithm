...
#ifdef  CCS_DESIGN_FUNC_matrix_mul
extern void mc_testbench_capture_IN( ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > A[32][32],  ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > B[32][32],  ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > C[32][32]);
extern void mc_testbench_capture_OUT( ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > A[32][32],  ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > B[32][32],  ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > C[32][32]);
extern void mc_testbench_wait_for_idle_sync();

#define ccs_intercept_matrix_mul_4 \
  ccs_real_matrix_mul(ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > A[32][32],ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > B[32][32],ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > C[32][32]);\
  void matrix_mul(ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > A[32][32],ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > B[32][32],ac_fixed<32, 18, true, AC_RND_CONV, AC_WRAP > C[32][32])\
{\
    static bool ccs_intercept_flag = false;\
    if (!ccs_intercept_flag) {\
      std::cout << "SCVerify intercepting C++ function 'matrix_mul' for RTL block 'matrix_mul'" << std::endl;\
      ccs_intercept_flag=true;\
    }\
    mc_testbench_capture_IN(A,B,C);\
    ccs_real_matrix_mul(A,B,C);\
    mc_testbench_capture_OUT(A,B,C);\
}\
  void ccs_real_matrix_mul
#else
#define ccs_intercept_matrix_mul_4 matrix_mul
#endif //CCS_DESIGN_FUNC_matrix_mul
