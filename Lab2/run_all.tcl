...
solution file add ./mult_add_pipeline_ref.cpp -type C++ -exclude true
solution file add ./mult_add_pipeline.cpp -type C++
go new
...
go analyze
directive set -DESIGN_HIERARCHY mult_add_pipeline
go compile
solution library add nangate-45nm_beh -file {$MGC_HOME/pkgs/siflibs/nangate/nangate-45nm_beh.lib} -- -rtlsyntool OasysRTL
go libraries
directive set -CLOCKS {clk {-CLOCK_PERIOD 1.11 -CLOCK_EDGE rising -CLOCK_UNCERTAINTY 0.0 -CLOCK_HIGH_TIME 0.555 -RESET_SYNC_NAME rst -RESET_ASYNC_NAME arst_n -RESET_KIND sync -RESET_SYNC_ACTIVE high -RESET_ASYNC_ACTIVE low -ENABLE_ACTIVE high}}
go assembly
...
go architect
go extract
#enable stalling in SCVerify
go new
solution file set ./tb.cpp -args -DSTALL
go extract
#enable flushing and turn of inputs in SCVerify
go new
solution options set /Flows/SCVerify/DISABLE_EMPTY_INPUTS true
go assembly
directive set /mult_add_pipeline/core/main -PIPELINE_STALL_MODE flush
go extract
#change IO to decoupled IO
go assembly
directive set /mult_add_pipeline/a:rsc -MAP_TO_MODULE ccs_ioport.ccs_in_wait_coupled
directive set /mult_add_pipeline/b:rsc -MAP_TO_MODULE ccs_ioport.ccs_in_wait_coupled
directive set /mult_add_pipeline/c:rsc -MAP_TO_MODULE ccs_ioport.ccs_in_wait_coupled
go extract