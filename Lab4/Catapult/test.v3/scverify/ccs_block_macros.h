...
#ifdef  CCS_DESIGN_FUNC_test
extern void mc_testbench_capture_IN( ac_channel<ac_int<10, true > > &data_in,  ac_int<7, true > coeffs[32][4],  ac_channel<ac_int<5, false > > &coeff_addr,  ac_channel<ac_int<19, true > > &result);
extern void mc_testbench_capture_OUT( ac_channel<ac_int<10, true > > &data_in,  ac_int<7, true > coeffs[32][4],  ac_channel<ac_int<5, false > > &coeff_addr,  ac_channel<ac_int<19, true > > &result);
extern void mc_testbench_wait_for_idle_sync();

#define ccs_intercept_test_4 \
  ccs_real_test(ac_channel<ac_int<10, true > > &data_in,ac_int<7, true > coeffs[32][4],ac_channel<ac_int<5, false > > &coeff_addr,ac_channel<ac_int<19, true > > &result);\
  void test(ac_channel<ac_int<10, true > > &data_in,ac_int<7, true > coeffs[32][4],ac_channel<ac_int<5, false > > &coeff_addr,ac_channel<ac_int<19, true > > &result)\
{\
    static bool ccs_intercept_flag = false;\
    if (!ccs_intercept_flag) {\
      std::cout << "SCVerify intercepting C++ function 'test' for RTL block 'test'" << std::endl;\
      ccs_intercept_flag=true;\
    }\
    mc_testbench_capture_IN(data_in,coeffs,coeff_addr,result);\
    ccs_real_test(data_in,coeffs,coeff_addr,result);\
    mc_testbench_capture_OUT(data_in,coeffs,coeff_addr,result);\
}\
  void ccs_real_test
#else
#define ccs_intercept_test_4 test
#endif //CCS_DESIGN_FUNC_test
